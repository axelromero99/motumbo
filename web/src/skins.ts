...
export const SKINS: SkinDef[] = [
  ...
  { name: 'Portugal', cat: 'bandera', draw: (g) => { g.fillStyle = '#006600'; g.fillRect(0, 0, W, H); g.fillStyle = '#da020e'; g.fillRect(W * 0.42, 0, W, H); both(g, (cx) => disc(g, cx, 12, '#ffcc00')); }, mat: glossy },
  { name: 'Grecia', cat: 'bandera', draw: (g) => { g.fillStyle = '#0d5eaf'; g.fillRect(0, 0, W, H); g.fillStyle = '#fff'; for (let i = 1; i < 9; i += 2) g.fillRect(0, (H / 9) * i, W, H / 9); both(g, (cx) => { g.fillStyle = '#fff'; g.fillRect(cx - 6, 8, 12, 40); g.fillRect(cx - 22, 22, 44, 12); }); }, mat: glossy },
  { name: 'Perú', cat: 'bandera', draw: (g) => bands(g, ['#d91023', '#ffffff', '#d91023'], true), mat: glossy },
  { name: 'Suecia', cat: 'bandera', draw: (g) => { g.fillStyle = '#006aa7'; g.fillRect(0, 0, W, H); g.fillStyle = '#fecc00'; g.fillRect(0, H * 0.42, W, H * 0.16); both(g, (cx) => g.fillRect(cx - 40, 0, 14, H)); }, mat: glossy },
  { name: 'Irlanda', cat: 'bandera', draw: (g) => bands(g, ['#169b62', '#ffffff', '#ff883e'], true), mat: glossy },
  ...
  ...([
    ...
    ['Corazones', (g: Ctx, c: string, d: string) => { g.fillStyle = c; g.fillRect(0, 0, W, H); g.fillStyle = d; for (let r = 0; r < 3; r++) for (let col = 0; col < 6; col++) { const x = col * 44 + (r % 2) * 22, y = r * 42 + 22; g.beginPath(); g.moveTo(x, y + 6); g.bezierCurveTo(x - 14, y - 10, x - 20, y + 8, x, y + 20); g.bezierCurveTo(x + 20, y + 8, x + 14, y - 10, x, y + 6); g.fill(); } }],
    ['Cebra', (g: Ctx, c: string, d: string) => { g.fillStyle = c; g.fillRect(0, 0, W, H); g.fillStyle = d; for (let x = -20; x < W; x += 28) { g.beginPath(); g.moveTo(x, 0); g.lineTo(x + 12, 0); g.lineTo(x + 12 + 22, H); g.lineTo(x + 22, H); g.closePath(); g.fill(); } }],
    ['Cuadros', (g: Ctx, c: string, d: string) => { g.fillStyle = c; g.fillRect(0, 0, W, H); g.fillStyle = d; for (let y = 0; y < 2; y++) for (let x = 0; x < 4; x++) if ((x + y) % 2 === 0) g.fillRect(x * 64, y * 64, 64, 64); }],
    ['Rombos', (g: Ctx, c: string, d: string) => { g.fillStyle = c; g.fillRect(0, 0, W, H); g.fillStyle = d; for (let r = 0; r < 4; r++) for (let col = 0; col < 6; col++) { const x = col * 44 + (r % 2) * 22, y = r * 34 + 16; g.beginPath(); g.moveTo(x, y - 13); g.lineTo(x + 13, y); g.lineTo(x, y + 13); g.lineTo(x - 13, y); g.closePath(); g.fill(); } }],
  ...
  { name: 'Oro', cat: 'material', draw: (g) => { g.fillStyle = '#c9a227'; g.fillRect(0, 0, W, H); }, mat: () => ({ color: new THREE.Color(0xffd94a), roughness: 0.22, metalness: 1, bumpMap: bump('metal'), bumpScale: 0.012 }) },
  { name: 'Cobre', cat: 'material', draw: (g) => { g.fillStyle = '#b87333'; g.fillRect(0, 0, W, H); }, mat: () => ({ color: new THREE.Color(0xd98a4a), roughness: 0.34, metalness: 1, bumpMap: bump('metal'), bumpScale: 0.014 }) },
  { name: 'Hielo', cat: 'material', draw: (g, col) => { g.fillStyle = hex(col); g.fillRect(0, 0, W, H); }, mat: (col) => ({ color: col, roughness: 0.06, metalness: 0.1, emissive: new THREE.Color(0x9fd8ff), emissiveIntensity: 0.12, bumpMap: bump('stone'), bumpScale: 0.03 }) },
  { name: 'Arcoíris', cat: 'material', draw: (g) => { const cols = ['#ff004d', '#ff8a00', '#ffe000', '#00c853', '#2979ff', '#8e24aa']; for (let i = 0; i < cols.length; i++) { g.fillStyle = cols[i]; g.fillRect(0, (H / cols.length) * i, W, H / cols.length + 1); } }, mat: () => ({ color: new THREE.Color(0xffffff), roughness: 0.3, metalness: 0.25 }) },
  { name: 'Circuito', cat: 'material', draw: (g) => { g.fillStyle = '#04160c'; g.fillRect(0, 0, W, H); g.strokeStyle = '#39ff14'; g.lineWidth = 2; g.fillStyle = '#39ff14'; for (let i = 0; i < 22; i++) { let x = (Math.random() * W) | 0, y = (Math.random() * H) | 0; g.beginPath(); g.moveTo(x, y); for (let k = 0; k < 3; k++) { if (Math.random() < 0.5) x += (Math.random() < 0.5 ? -1 : 1) * 30; else y += (Math.random() < 0.5 ? -1 : 1) * 24; g.lineTo(x, y); } g.stroke(); g.beginPath(); g.arc(x, y, 3, 0, 7); g.fill(); } }, mat: () => ({ color: new THREE.Color(0x0a3d1a), emissive: new THREE.Color(0x39ff14), emissiveIntensity: 0.32, roughness: 0.5 }) },
];
